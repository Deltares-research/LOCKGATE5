...
file_opt = options = {}
options['filetypes'] = [('KASGOLF input files', '.in'), ('all files', '.*')]
options['title'] = 'Select KASGOLF input file'
...
set_values = [0.0,1.0,2.0]#[1.0,0.5,2.0]#[0,1,2]
set_values2 = [1,0]#[1,0]
for i in range(0,1):#len(set_values)):
    ...
    #Test4
    Dict_inv['AO'] =set_values[i]# Dict_inv['AO']#
    #Test5
    Dict_inv['DT'] = Dict_inv['DT']#set_values[i]#
    #Test6
    Dict_inv['T1'] = Dict_inv['T1']#set_values#
    Dict_inv['N1'] = Dict_inv['N1']#set_values2#
...
plt.legend(['Smooth wave'])#['AO=0.0 [m2]','AO=1.0 [m2]','AO=2.0 [m2]'])#['BA=0.05 [m]','BA=0.30 [m]','BA=0.70 [m]'])#['BKAS=1.0 [m]','BKAS=0.5 [m]','BKAS=2.0 [m]'])#['M0=1 with VS=1.40 [m/s]','M0=0'])#,'DT=0.01 [s]','DT=0.10 [s]'],loc='upper left')
plt.xlim([-6,30])
plt.grid()
plt.xticks([-6,0,6,12,18,24,30])
