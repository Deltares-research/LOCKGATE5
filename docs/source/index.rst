...
LOCKGATE2 contains the codes KASGOLF and WINDGOLF. Both codes are discussed in this readthedocs. KASGOLF is extensively discussed in this readthedocs and a user guide to get started with the python code (translated from FORTRAN) is provided. For WINDGOLF the readthedocs is limited to the theory of the FORTRAN code only. Reason is that the theory behind WINDGOLF is outdated so the code is not translated to python and no user guide is provided.  
