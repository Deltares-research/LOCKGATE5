...
===========
This example demonstrates the capability of the KASGOLF code by applying it to an hypothetical scenario. An overview of the lock and lockgate can be found in the figure below. The simulation starts at time t=-6.0 [s] with a time step of 0.02 [s]. A wave arrives at location V at time t=0 [s] and reaches location W according to the propagation velocity that is calculated using shallow wave theory (M0=0). It is assumed that there is no horizontal opening underneath the gate (AO=0) and the friction coefficient (:math:`\mu`) for the discharge through the vertical and horizontal openings between the lock chamber and gate recess is equal to 0.70 [-]. 
